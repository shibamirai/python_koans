...
class AboutDecoratingWithClasses(Koan):
    ...
    def test_partial_that_wrappers_no_args(self):
        "引数をラップしない部分適用"
        """
        クラスによるデコレータにとりかかる前に、partial(部分適用)を知っておく必要があります
    ...
    class doubleit:
        ...
        def __get__(self, obj, cls=None):
            if not obj:
                # オブジェクトに束縛されていない関数を修飾(デコレート)します
                #  Decorating an unbound function
                return self
            else:
                # オブジェクトのメソッドを修飾(デコレート)します
    ...
    def test_decorator_with_no_arguments(self):
        # 補足：デコレートされた関数に引数があっても、このデコレータには引数がありません
