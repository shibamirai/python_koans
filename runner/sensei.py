...
class Sensei(MockableTestResult):
    ...
    def addSuccess(self, test):
        if self.passesCount():
            ...
            self.stream.writeln( \
                ...
                Fore.RESET, Style.NORMAL, test._testMethodDoc))
            self.pass_count += 1
    ...
    def learn(self):
        ...
        self.stream.writeln(
            "\n{0}**************************************************" \
            .format(Fore.RESET))
        self.stream.writeln("\n{0}おめでとうございます！すべてクリアしました。" \
            .format(Fore.MAGENTA))
        self.stream.writeln(
            "\nまだ物足りない場合は、about_extra_credit.py に挑戦してみてください{0}{1}" \
    ...
    def errorReport(self):
        ...
        test, err = problem
        self.stream.writeln("  {0}{1}{2} エラー".format(Fore.RED, Style.BRIGHT, test._testMethodName))
        ...
        self.stream.writeln("")
        self.stream.writeln("{0}{1}下記のコードを修正してください:" \
            .format(Fore.RESET, Style.NORMAL))
